...
def identifier_generator(wd_prop, list_name, descending_order=True):
    ...
    identifier_list = REDIS.hgetall(wd_prop + '_to_wikidata_20180804')
...
def doi_to_wikidata(doi):
    return hget('P356_to_wikidata_20180804', doi)

def pmid_to_wikidata(pmid):
    return hget('P698_to_wikidata_20180804', pmid)

def pmcid_to_wikidata(pmcid):
    return hget('P932_to_wikidata', pmcid)

def wikidata_to_doi(wikidata):
    return hget('wikidata_to_P356_20180804', wikidata)

def wikidata_to_pmid(wikidata):
    return hget('wikidata_to_P698_20180804', wikidata)
